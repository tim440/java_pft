...
@Story("Удаление")
public class RemoveAgentStatementTests extends TestBase {
  ...
  @Test(description = "Удаление рублевого ОА")
  public void removeAgentStatementRuTest() {
    String date = CurrentData();
    searchAgentStatement("72850010-72850010" + CreateObjects.getAgentStatementDataRu(date));
  ...
  @Test(description = "Удаление валютного ОА")
  public void removeAgentStatementUsdTest() {
    storage.setSessionCookie(getSessionCookie());
    String date = CurrentData();
    searchAgentStatement("72850010-72850010" + CreateObjects.getAgentStatementDateUSD(date));
    AgentStatementPreview agentStatementSearch = storage.getAgentStatementSearch();
    String statementId = agentStatementSearch.getDataList().get(0).getId();
    removeAgentStatement(statementId);
  }
  ...
  private void responseStatementRemove(String requestAgentStatementDel) {
    ...
    assertEquals(deleteAgentStatement.isSuccess(), true);
  }
